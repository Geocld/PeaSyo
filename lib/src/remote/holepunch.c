...
CHIAKI_EXPORT ChiakiErrorCode chiaki_holepunch_session_create(Session* session)
{
    ...
    CHIAKI_LOGE(session->log, "chiaki_holepunch_session_create: Created websocket thread");

    chiaki_mutex_lock(&session->state_mutex);
    while (!(session->state & SESSION_STATE_WS_OPEN))
    {
        CHIAKI_LOGE(session->log, "chiaki_holepunch_session_create: Waiting for websocket to open...");
        ...
        ChiakiHolepunchConsoleType console_type)
{
    ...
    else
    {
        ...
        CHIAKI_LOGE(session->log, "chiaki_holepunch_session_start: Starting holepunch session %s for the PS5 console with duid %s", session->session_id, duid_str);
...
static ChiakiErrorCode http_ps4_session_wakeup(Session *session)
{
    ...
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "http_ps4_session_wakeup: CURL setopt CURLOPT_SSL_VERIFYPEER failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "http_ps4_session_wakeup: CURL setopt CURLOPT_SSL_VERIFYHOST failed with CURL error %s", curl_easy_strerror(res));
...
/**
 * Get the f-ully qualified domain name of the websocket server that we can
...
static ChiakiErrorCode get_websocket_fqdn(Session *session, char **fqdn)
{
    ...
    CURLcode res = curl_easy_setopt(curl, CURLOPT_SHARE, session->curl_share);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch get_websocket_fqdn: CURL setopt CURLOPT_SHARE failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_FAILONERROR, 1L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch get_websocket_fqdn: CURL setopt CURLOPT_FAILONERROR failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_TIMEOUT, 10L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch get_websocket_fqdn: CURL setopt CURLOPT_TIMEOUT failed with CURL error %s", curl_easy_strerror(res));
    ...
    res = curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, curl_write_cb);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch get_websocket_fqdn: CURL setopt CURLOPT_WRITEFUNCTION failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_WRITEDATA, (void*)&response_data);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch get_websocket_fqdn: CURL setopt CURLOPT_WRITEDATA failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch get_websocket_fqdn: CURL setopt CURLOPT_SSL_VERIFYPEER failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch get_websocket_fqdn: CURL setopt CURLOPT_SSL_VERIFYHOST failed with CURL error %s", curl_easy_strerror(res));
    ...
    if (res != CURLE_OK)
    {
        if (res == CURLE_HTTP_RETURNED_ERROR)
        {
            long http_code = 0;
            curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &http_code);
            CHIAKI_LOGE(session->log, "holepunch get_websocket_fqdn: Fetching websocket FQDN from %s failed with HTTP code %ld", ws_fqdn_api_url, http_code);
            err = CHIAKI_ERR_HTTP_NONOK;
        } else {
            CHIAKI_LOGE(session->log, "holepunch get_websocket_fqdn: Fetching websocket FQDN from %s failed with CURL error %s", ws_fqdn_api_url, curl_easy_strerror(res));
            err = CHIAKI_ERR_NETWORK;
        }
        goto cleanup;
    }
    ...
    if(!tok)
    {
        CHIAKI_LOGE(session->log, "holepunch Couldn't create new json tokener");
        goto cleanup;
    }
    json_object *json = json_tokener_parse_ex(tok, response_data.data, response_data.size);
    if (json == NULL)
    {
        CHIAKI_LOGE(session->log, "holepunch get_websocket_fqdn: Parsing JSON failed");
        err = CHIAKI_ERR_UNKNOWN;
        goto cleanup_json_tokener;
    }
    ...
    {
        CHIAKI_LOGE(session->log, "holepunch get_websocket_fqdn: JSON does not contain \"fqdn\" field");
        err = CHIAKI_ERR_UNKNOWN;
        goto cleanup_json;
    } else if (!json_object_is_type(fqdn_json, json_type_string))
    {
        CHIAKI_LOGE(session->log, "holepunch get_websocket_fqdn: JSON \"fqdn\" field is not a string");
        err = CHIAKI_ERR_UNKNOWN;
        goto cleanup_json;
    }
...
static void* websocket_thread_func(void *user) {
    ...
    CHIAKI_LOGE(session->log, "holepunch websocket_thread_func");
    ...
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch websocket_thread_func: CURL setopt CURLOPT_HTTPHEADER failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_SHARE, session->curl_share);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch websocket_thread_func: CURL setopt CURLOPT_SHAREfailed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_FAILONERROR, 1L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch websocket_thread_func: CURL setopt CURLOPT_FAILONERROR failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_TIMEOUT, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch websocket_thread_func: CURL setopt CURLOPT_TIMEOUT failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_URL, ws_url);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch websocket_thread_func: CURL setopt CURLOPT_URL failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_CONNECT_ONLY, 2L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch websocket_thread_func: CURL setopt CURLOPT_CONNECT_ONLY failed with CURL error %s", curl_easy_strerror(res));

    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch websocket_thread_func: CURL setopt CURLOPT_SSL_VERIFYPEER failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "holepunch websocket_thread_func: CURL setopt CURLOPT_SSL_VERIFYHOST failed with CURL error %s", curl_easy_strerror(res));
    ...
    if (res != CURLE_OK)
    {
        if (res == CURLE_HTTP_RETURNED_ERROR)
        {
            long http_code = 0;
            curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &http_code);
            CHIAKI_LOGE(session->log, "holepunch  websocket_thread_func: Connecting to push notification WebSocket %s failed with HTTP code %ld", ws_url, http_code);
        } else {
            CHIAKI_LOGE(session->log, "holepunch  websocket_thread_func: Connecting to push notification WebSocket res: %d", res);
            CHIAKI_LOGE(session->log, "holepunch  websocket_thread_func: Connecting to push notification WebSocket %s failed with CURL error %s", ws_url, curl_easy_strerror(res));
        }
        goto cleanup;
    }
    session->ws_open = true;
    CHIAKI_LOGE(session->log, "holepunch websocket_thread_func: Connected to push notification WebSocket %s", ws_url);
    ...
    if(res != CURLE_OK) {
        CHIAKI_LOGE(session->log, "holepunch websocket_thread_func: Getting active socket failed with CURL error %s", curl_easy_strerror(res));
        goto cleanup;
    }
    fd_set fds;
    FD_ZERO(&fds);
    ...
    while (!session->ws_thread_should_stop)
    {
        ...
        {
            CHIAKI_LOGE(session->log, "holepunch websocket_thread_func: Did not receive PONG in time.");
            goto cleanup_json;
        }
...
static ChiakiErrorCode http_create_session(Session *session)
{
    ...
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "http_create_session: CURL setopt CURLOPT_SSL_VERIFYPEER failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "http_create_session: CURL setopt CURLOPT_SSL_VERIFYHOST failed with CURL error %s", curl_easy_strerror(res));
...
static ChiakiErrorCode http_check_session(Session *session, bool viewurl)
{
    ...
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "http_check_session: CURL setopt CURLOPT_SSL_VERIFYPEER failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "http_check_session: CURL setopt CURLOPT_SSL_VERIFYHOST failed with CURL error %s", curl_easy_strerror(res));
...
static ChiakiErrorCode http_start_session(Session *session)
{
    ...
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "http_start_session: CURL setopt CURLOPT_SSL_VERIFYPEER failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "http_start_session: CURL setopt CURLOPT_SSL_VERIFYHOST failed with CURL error %s", curl_easy_strerror(res));

    CHIAKI_LOGE(session->log, "holepunch http_start_session: Sending JSON:\n%s", envelope_buf);

    res = curl_easy_perform(curl);
    curl_slist_free_all(headers);
    CHIAKI_LOGE(session->log, "holepunch http_start_session: Received JSON:\n%.*s", (int)response_data.size, response_data.data);
    if (res != CURLE_OK)
    {
        if (res == CURLE_HTTP_RETURNED_ERROR)
        {
...
static ChiakiErrorCode http_send_session_message(Session *session, SessionMessage *message, bool short_msg)
{
    ...
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "http_send_session_message: CURL setopt CURLOPT_SSL_VERIFYPEER failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "http_send_session_message: CURL setopt CURLOPT_SSL_VERIFYHOST failed with CURL error %s", curl_easy_strerror(res));
...
static ChiakiErrorCode deleteSession(Session *session)
{
    ...
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "delete_session: CURL setopt CURLOPT_SSL_VERIFYPEER failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "delete_session: CURL setopt CURLOPT_SSL_VERIFYHOST failed with CURL error %s", curl_easy_strerror(res));
    ...
    while (true) {
        while (session->ws_notification_queue->rear == last_known || session->ws_notification_queue->rear == NULL)
        {
            ...
            if (err == CHIAKI_ERR_TIMEOUT)
            {
                ...
                {
                    CHIAKI_LOGE(session->log, "holepunch wait_for_notification: Timed out waiting for holepunch session messages");
        ...
        while (notif != NULL && notif != last_known)
        {
            if (notif->type & types)
            {
                CHIAKI_LOGV(session->log, "holepunch wait_for_notification: Found notification of type %d", notif->type);
                *out = notif;
...
static ChiakiErrorCode get_stun_servers(Session *session)
{
    ...
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "get_stun_servers: CURL setopt CURLOPT_SSL_VERIFYPEER failed with CURL error %s", curl_easy_strerror(res));
    res = curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0L);
    if(res != CURLE_OK)
        CHIAKI_LOGW(session->log, "get_stun_servers: CURL setopt CURLOPT_SSL_VERIFYHOST failed with CURL error %s", curl_easy_strerror(res));
