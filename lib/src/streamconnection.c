...
stream_connection->last_send_time = 0;
...
static void stream_connection_takion_data_protobuf(ChiakiStreamConnection *stream_connection, uint8_t *buf, size_t buf_size)
{
    ...
    // CHIAKI_LOGE(stream_connection->log, "stream_connection_takion_data_protobuf: %d", stream_connection->state);
...
float alpha = 0.125f; // 平滑因子，可以根据需求调整
float smoothed_rtt = 0;
float calculate_smooth_rtt(float new_rtt) {
    if (smoothed_rtt == 0) {
        smoothed_rtt = new_rtt;
    } else {
        smoothed_rtt = (1 - alpha) * smoothed_rtt + alpha * new_rtt;
    }

    // 如果值异常大，可能是秒单位，需要转换
    if (smoothed_rtt < 1.0) {
        return smoothed_rtt * 1000; // 转换为毫秒
    } else if (smoothed_rtt > 1000) {
        return smoothed_rtt / 1000; // 可能是微秒，转换为毫秒
    }
...
static void stream_connection_takion_data_idle(ChiakiStreamConnection *stream_connection, uint8_t *buf, size_t buf_size)
{
    ...
    CHIAKI_LOGE(stream_connection->log, "StreamConnection received data with msg.type == %d", msg.type);
    ...
    switch (msg.type) {
        case tkproto_TakionMessage_PayloadType_DISCONNECT: // 8
        ...
        case tkproto_TakionMessage_PayloadType_HEARTBEAT:
            CHIAKI_LOGE(stream_connection->log, "receive HEARTBEAT");
            uint64_t current_time = chiaki_time_now_monotonic_us();
            if (stream_connection->last_send_time > 0) {
                float measured_rtt = (float)(current_time - stream_connection->last_send_time) / 1000.0f;
                stream_connection->last_send_time = 0;
                CHIAKI_LOGE(stream_connection->log, "measured_rtt : %.2f", measured_rtt);
                stream_connection->rtt = measured_rtt;
            }
            break;
        case tkproto_TakionMessage_PayloadType_CONNECTIONQUALITY: // 16
        {
            // CHIAKI_LOGE(stream_connection->log, "tkproto_TakionMessage_PayloadType_CONNECTIONQUALITY");
            tkproto_ConnectionQualityPayload q = msg.connection_quality_payload;
            // 性能检测，将性能信息传递到android端使用
            CHIAKI_LOGE(
            ...
            CHIAKI_LOGE(stream_connection->log, "StreamConnection measured bitrate: %.4f MBit/s", stream_connection->measured_bitrate);
...
static ChiakiErrorCode stream_connection_send_heartbeat(ChiakiStreamConnection *stream_connection)
{
    ...
    CHIAKI_LOGE(stream_connection->log, "stream_connection_send_heartbeat");
    stream_connection->last_send_time = chiaki_time_now_monotonic_us();
