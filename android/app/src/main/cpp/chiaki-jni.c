...
typedef struct android_chiaki_session_t
{
    ...
    // Kotlin: Session.eventHapticAudio(byte[])
    jmethodID java_session_event_haptic_audio_meth;
...
JNIEXPORT jdouble JNICALL JNI_FCN(getRtt)(JNIEnv *env, jobject obj, jlong ptr)
{
    ...
    if(rtt <= 0.0 && session->session.rtt_us > 0)
        rtt = session->session.rtt_us / 1000.0;
    return rtt;
}
