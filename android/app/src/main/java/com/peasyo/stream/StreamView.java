...
class OrientationTracker {
    ...
    private static final float BETA_DEFAULT = 90000f;
...
//        Log.d(TAG, "handleKeyEvent:" +  event);
    ...
    public void handleUsbDsControllerEvent(
        ...
        if (lastUpdateTime != 0) {
            deltaTimeInSeconds = (currentTime - lastUpdateTime) * 1e-9f;
        } else {
            deltaTimeInSeconds = 0.01f; // 首次运行时使用默认值
        }

        float[] quaternion = new float[4];
        quaternion = tracker.updateOrientation(
                gyrox, gyroy, gyroz,          // 陀螺仪数据（弧度/秒）
                accel0, accel1, accel2,       // 加速度计数据（m/s²）
                deltaTimeInSeconds             // 时间步长
        );
...
//        controllerState.setOrientW(1.0f);
        ...
        controllerState.setOrientX(-x);
        controllerState.setOrientY(y);
        controllerState.setOrientZ(x);
        controllerState.setOrientW(-w);
