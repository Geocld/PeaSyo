...
public class XboxOneController extends AbstractXboxController {
        ...
        switch (buffer.get())
        {
            case 0x20:
                if (buffer.remaining() < 17) {
                    Log.d("UsbDriverService XboxOneController.java", "XBone button/axis read too small: "+buffer.remaining());
                    return false;
                }
            ...
            case 0x07:
                if (buffer.remaining() < 4) {
                    Log.d("UsbDriverService XboxOneController.java", "XBone mode read too small: "+buffer.remaining());
                    return false;
                }
    ...
    protected boolean doInit() {
        ...
        for (InitPacket pkt : INIT_PKTS) {
            ...
            if (res != data.length) {
                Log.d("UsbDriverService XboxOneController.java", "Initialization transfer failed: "+res);
                return false;
            }
        }
    ...
    private void sendRumblePacket() {
        ...
        if (res != data.length) {
            Log.d("UsbDriverService XboxOneController.java", "Rumble transfer failed: "+res);
        }
    }
